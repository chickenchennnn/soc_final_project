	...
	always@(posedge axis_clk) begin
		if(!axis_rst_n) begin
			...
			if(ss_tready && ss_tvalid) begin
				...
				save_data[1] <= save_data[0];
				save_data[2] <= save_data[1];
				save_data[3] <= save_data[2];
				save_data[4] <= save_data[3];
				save_data[5] <= save_data[4];
				save_data[6] <= save_data[5];
				save_data[7] <= save_data[6];
				save_data[8] <= save_data[7];
				save_data[9] <= save_data[8];
			end else begin
				save_data[0] <= save_data[0];
				save_data[1] <= save_data[1];
				save_data[2] <= save_data[2];
				save_data[3] <= save_data[3];
				save_data[4] <= save_data[4];
				save_data[5] <= save_data[5];
				save_data[6] <= save_data[6];
				save_data[7] <= save_data[7];
